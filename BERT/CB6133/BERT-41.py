...
t = text.Transformer(MODEL_NAME, maxlen=41, classes=[0, 1, 2, 3, 4, 5, 6, 7])
trn = t.preprocess_train(f_X_val, f_y_val)
val = t.preprocess_test(f_X_val, f_y_val)
model = t.get_classifier()

learner = ktrain.get_learner(model, train_data=trn, val_data=val, batch_size=32)
learner.autofit(1e-5, 5, early_stopping=1, checkpoint_folder='/content/drive/MyDrive/prot_bert_bfd_cb6133/41')
predictor = ktrain.get_predictor(learner.model, preproc=t)

predict = predictor.predict_proba(f_X_val)
predict = np.array(predict)
np.save('/content/drive/MyDrive/prot_bert_bfd_cb6133/bertprot-cb6133-window-41-val.npy', predict)

predict = predictor.predict_proba(f_X_test)
predict = np.array(predict)
np.save('/content/drive/MyDrive/prot_bert_bfd_cb6133/bertprot-cb6133-window-41-test.npy', predict)