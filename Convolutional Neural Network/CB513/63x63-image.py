...
for idx in tqdm(range(1, 5279)):
  img, labels = dados(31, idx-1, idx)
  img = np.reshape(img, (-1, 63, 63, 1))
...
mc = tf.keras.callbacks.ModelCheckpoint('/content/drive/MyDrive/PSSM/CB513/EfficientNetB7-63x63x3.hdf5', monitor='val_loss', save_best_only=True, verbose=1)
...
np.save('/content/drive/MyDrive/PSSM/CB513/pred_efficientnetb7/63x63x3-val.npy', pred)

# test
pred = model.predict(test_gen, batch_size=32, verbose=1)
np.save('/content/drive/MyDrive/PSSM/CB513/pred_efficientnetb7/63x63x3-test.npy', pred)