...
for idx in tqdm(range(1, 5279)):
  img, labels = dados(73, idx-1, idx)
  img = np.reshape(img, (-1, 147, 147, 1))
  ...
  for index in range(len(img)):
    ...
    cv2.imwrite('/content/CB513/Train/train_img'+ str(cont_img) + '.png', a)
...
df = pd.DataFrame(dic)
df.to_csv('/content/CB513/train.csv', index=False)
...
for idx in tqdm(range(5279, 5535)):
  img, labels = dados(73, idx-1, idx)
  img = np.reshape(img, (-1, 147, 147, 1))
  labels = np.argmax(labels, axis=1)
  ...
  for index in range(len(img)):
    ...
    a = a.astype(np.uint8)
    a = cv2.cvtColor(a, cv2.COLOR_GRAY2RGB)
    cv2.imwrite('/content/CB513/Val/val_img'+ str(cont_img) + '.png', a)
...
dic = {'Id': img_name, 'Target': img_target}
df = pd.DataFrame(dic)
df.to_csv('/content/CB513/val.csv', index=False)
...
for idx in tqdm(range(1, 515)):
  ...
  for index in range(len(img)):
    ...
    a = a.astype(np.uint8)
    a = cv2.cvtColor(a, cv2.COLOR_GRAY2RGB)
    cv2.imwrite('/content/CB513/Test/test_img'+ str(cont_img) + '.png', a)
...
dic = {'Id': img_name, 'Target': img_target}
df = pd.DataFrame(dic)
df.to_csv('/content/CB513/test.csv', index=False)



# loading dataframes with images path
df_train = pd.read_csv('/content/CB513/train.csv')
df_val = pd.read_csv('/content/CB513/val.csv')
df_train['Target'] = df_train['Target'].astype('str')
df_val['Target'] = df_val['Target'].astype('str')
df_teste = pd.read_csv('/content/CB513/test.csv')
...
train_gen = datagen.flow_from_dataframe(
      df_train, directory='/content/CB513/Train', x_col='Id', y_col='Target',
      target_size=(147, 147), class_mode='categorical', batch_size=32, shuffle=True, seed=42)

val_gen = datagen.flow_from_dataframe(
      df_val, directory='/content/CB513/Val', x_col='Id', y_col='Target',
      target_size=(147, 147), class_mode='categorical', batch_size=32, shuffle=False, seed=42)

test_gen = datagen.flow_from_dataframe(
      df_teste, directory='/content/CB513/Test', x_col='Id', y_col='Target',
      target_size=(147, 147), class_mode='categorical', batch_size=32, shuffle=False, seed=42)


# generate the model
cnn = tf.keras.applications.EfficientNetB7(weights='imagenet', include_top=False, input_shape=(147,147,3))
...
mc = tf.keras.callbacks.ModelCheckpoint('/content/drive/MyDrive/PSSM/CB513/EfficientNetB7-147x147x3.hdf5', monitor='val_loss', save_best_only=True, verbose=1)
...
np.save('/content/drive/MyDrive/PSSM/CB513/pred_efficientnetb7/147x147x3-val.npy', pred)

# test
pred = model.predict(test_gen, batch_size=32, verbose=1)
np.save('/content/drive/MyDrive/PSSM/CB513/pred_efficientnetb7/147x147x3-test.npy', pred)